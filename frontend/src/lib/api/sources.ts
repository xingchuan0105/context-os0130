...
export const sourcesApi = {
  ...
  get: async (id: string) => {
    ...
    const response = await apiClient.get<SourceDetailResponse>(`/sources/${normalizedId}`)
    return response.data
  },
  ...
  update: async (id: string, data: UpdateSourceRequest) => {
    const normalizedId = normalizeSourceId(id)
    const response = await apiClient.put<SourceListResponse>(`/sources/${normalizedId}`, data)
    return response.data
  },

  delete: async (id: string, params?: { notebook_id?: string }) => {
    const normalizedId = normalizeSourceId(id)
    await apiClient.delete(`/sources/${normalizedId}`, { params })
  },

  status: async (id: string) => {
    const normalizedId = normalizeSourceId(id)
    const response = await apiClient.get<SourceStatusResponse>(`/sources/${normalizedId}/status`)
    return response.data
  },
  ...
  retry: async (id: string) => {
    const normalizedId = normalizeSourceId(id)
    const response = await apiClient.post<SourceResponse>(`/sources/${normalizedId}/retry`)
    return response.data
  },

  downloadFile: async (id: string): Promise<AxiosResponse<Blob>> => {
    const normalizedId = normalizeSourceId(id)
    return apiClient.get(`/sources/${normalizedId}/download`, {
      responseType: 'blob',
    })
  },
}
