...
export const notebooksApi = {
  ...
  addSource: async (notebookId: string, sourceId: string) => {
    ...
    const response = await apiClient.post(`/notebooks/${notebookId}/sources/${normalizedId}`)
    return response.data
  },

  removeSource: async (notebookId: string, sourceId: string) => {
    const normalizedId = normalizeSourceId(sourceId)
    const response = await apiClient.delete(`/notebooks/${notebookId}/sources/${normalizedId}`)
    return response.data
  }
}
