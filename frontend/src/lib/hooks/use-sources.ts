  ...
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const hasProcessing = data.some((source) => {
        ...
        return status === 'queued' || status === 'processing' || status === 'running' || status === 'new'
      })
...
export function useDeleteSource() {
  ...
  const normalizeDeleteInput = (input: string | { id: string; notebookId?: string }) => {
    if (typeof input === 'string') {
      return { id: input, notebookId: undefined }
    }
    return { id: input.id, notebookId: input.notebookId }
  }

  return useMutation({
    mutationFn: (input: string | { id: string; notebookId?: string }) => {
      const { id, notebookId } = normalizeDeleteInput(input)
      return sourcesApi.delete(id, notebookId ? { notebook_id: notebookId } : undefined)
    },
    onSuccess: (_, input) => {
      const { id, notebookId } = normalizeDeleteInput(input)
      // Invalidate ALL sources queries (both general and notebook-specific)
...
export function useSourceStatus(sourceId: string, enabled = true) {
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      if (data?.status === 'running' || data?.status === 'processing' || data?.status === 'queued' || data?.status === 'new') {
        return 2000
      }
