...
export function TransformationEditorDialog({ open, onOpenChange, transformation }: TransformationEditorDialogProps) {
  ...
  const transformationSchema = z.object({
    name: z.string().min(1, t('transformations.error.nameRequired')),
    title: z.string().optional(),
    description: z.string().optional(),
    prompt: z.string().min(1, t('transformations.error.promptRequired')),
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-4xl w-full max-h-[90vh] overflow-hidden p-0">
        <DialogTitle className="sr-only">
          {isEditing ? t('transformations.editTitle') : t('transformations.createTitle')}
        </DialogTitle>
        <form onSubmit={handleSubmit(onSubmit)} className="flex h-full flex-col">
          {isEditing && isLoading ? (
            <div className="flex-1 flex items-center justify-center py-10">
              <span className="text-sm text-muted-foreground">{t('transformations.loading')}</span>
            </div>
          ) : (
            <>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        {...field}
                        placeholder={t('transformations.placeholder.name')}
                      />
                    )}
                  />
                  {errors.name && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Input
                          ...
                          {...field}
                          placeholder={t('transformations.placeholder.title')}
                        />
                      )}
                    />
                  </div>
                  <div className="flex items-center gap-2 pt-6 md:pt-8">
                    ...
                    <Label htmlFor="transformation-default" className="text-sm">
                      {t('transformations.defaultSuggestion')}
                    </Label>
                  </div>
                </div>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Textarea
                        ...
                        {...field}
                        placeholder={t('transformations.placeholder.description')}
                        rows={2}
                      />
                    )}
                  />
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4">
                <Label className="text-sm font-medium">{t('transformations.prompt')}</Label>
                <Controller
                  control={control}
                  name="prompt"
                  render={({ field }) => (
                    <MarkdownEditor
                      ...
                      height={420}
                      placeholder={t('transformations.placeholder.prompt')}
          ...
          <div className="border-t px-6 py-4 flex justify-end gap-2">
            ...
            <Button type="submit" disabled={isSaving || (isEditing && isLoading)}>
              {isSaving
                ? isEditing ? t('transformations.saving') : t('transformations.creating')
                : isEditing
                  ? t('transformations.save')
                  : t('transformations.create')}
            </Button>
          </div>
        </form>
      </DialogContent>
